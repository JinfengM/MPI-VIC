...
int main(int argc, char** argv) {
  ...
  for (i = 0; i < NUM_SLAVES; i++) {
    recv_end_msgs[i] = (char*)malloc((strlen(end_msg) + 1) * sizeof(char));
  ...
  if (world_rank == MASTER_RANK) {
    ...
    for (i = 1; i <= NUM_SLAVES; i++) {
      MPI_Irecv(recv_end_msgs[i-1], strlen(end_msg) + 1, MPI_CHAR, i, tag_end, MPI_COMM_WORLD, &requests[NUM_SLAVES + i - 1]);
      }
  ...
  } else
  {
    ...
    char recv_start_msg[sizeof(start_msg)];
    MPI_Request request;
    MPI_Recv(recv_start_msg, sizeof(recv_start_msg), MPI_CHAR, MASTER_RANK, tag_start, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    ...
    if (strcmp(recv_start_msg, "START") == 0) {
        ...
        vicmain(world_rank,6,vars,outputdir,startLine,endLine);
